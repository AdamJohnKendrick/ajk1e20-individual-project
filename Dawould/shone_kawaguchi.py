...
encoder_first = Dense(encoding_dim, activation="relu")(input_layer)
encoder_first = Dense(128, activation="relu")(encoder_first)

#decoding and output layer

decoder_first = Dense(64, activation='softmax')(encoder_first)
decoder_first = Dense(32, activation='softmax')(decoder_first)

encoder_second = Dense(14, activation='relu')(decoder_first)
latent_space = Dense(32)(encoder_second)

decoder_second = Dense(28, activation='relu')(encoder_second)
decoder_second = Dense(41, activation='relu')(encoder_second)

# Define the special neuron
special = Lambda(lambda x: K.ones_like(x))(latent_space)

# # creating model with input, encoding, decoding, output layers
# autoencoder = Model(inputs=input_layer, outputs=decoder_second)

# Define the model with the additional constraint
lambda_val = 0.1
model = Model(inputs=[input_layer], outputs=[decoder_second])
...
# pred_train = autoencoder.predict(X_train, X_train)
# pred_test = autoencoder.predict(X_test, X_test)

# #Random Forest
# rfc = RandomForestClassifier(n_estimators=100,
#                              max_depth=10,
#                              random_state=42) # Adjust hyperparameters as desired
# rfc.fit(pred_train, X_train)
#
# y_pred = rfc.predict(pred_test)
