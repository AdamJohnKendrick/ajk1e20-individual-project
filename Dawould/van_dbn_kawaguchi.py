...
# Add a special neuron per output unit to eliminate suboptimal local minima
rbm.add(Dense(2, activation='linear', use_bias=False))
rbm.add(Lambda(lambda x: tf.nn.l2_normalize(x, axis=1)))

# Compile the stacked RBM mode
rbm